...
const Index = () => {
  ...
  const kpis = useMemo(() => {
    if (!data) return { totalSpend: 0, campaignCount: 0, accountCount: 0, avgSpendPerCampaign: 0 };
    const totalSpend = data.reduce((s, r) => s + (r.spend ?? 0), 0);
    const campaigns = new Set(data.map((r) => r["Campaign name"]).filter(Boolean));
    const accounts = new Set(data.map((r) => r["Account name"]));
    return {
      totalSpend,
      campaignCount: campaigns.size,
      accountCount: accounts.size,
      avgSpendPerCampaign: campaigns.size > 0 ? totalSpend / campaigns.size : 0,
    };
  }, [data]);
